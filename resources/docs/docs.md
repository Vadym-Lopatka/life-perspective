...
The `home-routes` handler in the `life-perspective.routes.home` namespace
...
The `home-page` function will in turn call the `life-perspective.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `life-perspective.handler` namespace:
...
Request middleware functions are located under the `life-perspective.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `life-perspective.dev-middleware` namespace found in
...
* Let `mount` know to start the database connection by `require`-ing `life-perspective.db.core` in some other namespace.
